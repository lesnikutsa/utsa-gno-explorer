...
export function CosmosValidatorIdentity({ moniker, address, imageSrc }) {
  ...
  return <span className="cosmos-validator-identity">
    ...
    <span className="cosmos-validator"><strong title={moniker}>{moniker || 'Unknown proposer'}</strong><span className="mono muted" title={address}>{shortAddress(address)}</span></span>
  </span>
}
