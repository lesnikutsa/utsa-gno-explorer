...
export function FutureBlockCard({ data, height, now }) {
  ...
  const remaining = eta?.remaining_blocks ?? Number(height) - data.current_height
  return <section className="cosmos-card cosmos-block-state"><h2>Block #{Number(height).toLocaleString()} has not been produced yet</h2>
    ...
    <div className="cosmos-detail-summary cosmos-future-metrics"><Metric label="Current height">{data.current_height.toLocaleString('en-US')}</Metric><Metric label="Blocks remaining">{remaining.toLocaleString('en-US')}</Metric><Metric label="Average block time">{eta ? formatAverageBlockTime(eta.average_block_seconds) : '—'}</Metric><Metric label="Estimated arrival">{eta ? formatEstimatedArrival(eta.estimated_at) : '—'}</Metric></div>
