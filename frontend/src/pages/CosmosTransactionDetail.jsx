...
export function CosmosTransactionDetail({ network, height, index }) {
  ...
  return <div className="cosmos-block-detail cosmos-transaction-detail"><a className="cosmos-back block-detail__back" href={`/networks/${network.id}/blocks/${height}`}>← Back to Block #{Number(height).toLocaleString()}</a><div className="cosmos-title"><h1>Transaction #{tx.index}</h1></div>
    <section className="cosmos-detail-summary panel"><div><span>Status</span><strong><span className={`cosmos-tx-status cosmos-tx-status--${tx.success ? 'success' : 'failed'}`}>{tx.success ? 'Success' : 'Failed'}</span></strong></div><div><span>Block</span><strong><a className="table-link accent-value" href={`/networks/${network.id}/blocks/${tx.height}`}>#{tx.height.toLocaleString()}</a></strong></div><div><span>Time</span><strong><time dateTime={tx.timestamp} title={tx.timestamp}>{utc(tx.timestamp)}</time></strong></div></section>
