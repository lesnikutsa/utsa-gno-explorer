...
function Metadata({ metadata }) {
  ...
  return (
    <section className="panel realm-detail__section realm-metadata" aria-labelledby="realm-metadata-title">
      ...
      <div className="realm-metadata__source"><h3>Source</h3>{sourceFile && <p className="mono">{sourceFile.filename} · {formatBytes(sourceFile.byte_count)} · {formatCount(sourceFile.line_count)} lines</p>}<button className="blocks-page__button" type="button" aria-expanded={sourceExpanded} onClick={() => setSourceExpanded((expanded) => !expanded)} disabled={!sourceFile}>{sourceExpanded ? 'Hide source' : 'Show source'}</button>{metadata.source.loading && <p>Loading source…</p>}{metadata.source.error && <p>Source file is temporarily unavailable.</p>}{sourceExpanded && metadata.source.data && <pre><code>{metadata.source.data.content}</code></pre>}</div>
    </section>
  )
}
