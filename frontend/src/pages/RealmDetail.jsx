  ...
  return (
    <section className="panel realm-detail__section realm-metadata" aria-labelledby="realm-metadata-title">
      ...
      <div className="realm-metadata__grid">
        ...
        <div><h3>Files <span>{formatCount(summary.file_count)}</span></h3><ul className="realm-metadata__files">{data.files.map((file) => <li key={file.filename}><button className={file.filename === metadata.selectedFilename ? 'is-selected' : undefined} type="button" aria-pressed={file.filename === metadata.selectedFilename} onClick={() => metadata.selectFile(file.filename)}><span className="mono">{file.filename}</span><StatusBadge tone="neutral">{fileKindLabel[file.file_kind]}</StatusBadge><small>{formatBytes(file.byte_count)} · {formatCount(file.line_count)} lines</small></button></li>)}</ul></div>
      ...
      <div className="realm-metadata__source"><h3>Source</h3><div className="realm-metadata__source-header">{sourceFile && <p className="mono">{sourceFile.filename} · {formatBytes(sourceFile.byte_count)} · {formatCount(sourceFile.line_count)} lines</p>}<button className="blocks-page__button realm-metadata__source-toggle" type="button" aria-expanded={sourceExpanded} onClick={() => setSourceExpanded((expanded) => !expanded)} disabled={!sourceFile}>{sourceExpanded ? 'Hide source ↑' : 'Show source ↓'}</button></div>{metadata.source.loading && <p>Loading source…</p>}{metadata.source.error && <p>Source file is temporarily unavailable.</p>}{sourceExpanded && metadata.source.data && <pre><code>{metadata.source.data.content}</code></pre>}</div>
    </section>
  )
}
