...
export function CosmosTransactions({ network }) {
  ...
  return <div className="cosmos-blocks cosmos-transactions"><div className="cosmos-title"><h1>Transactions</h1>{resource.stale || resource.error
    ...
    {state === 'available' && <><section className="panel cosmos-blocks-table"><div className="cosmos-table"><table><thead><tr><th>Type</th><th>Tx hash</th><th>Time</th><th>Block</th><th>Status</th><th>Fee</th><th>Gas</th></tr></thead><tbody>{rows.length ? rows.map((row, index) => <tr key={row.tx_hash} className={!cursor && row.tx_hash === newest && index === 0 ? 'is-new-row' : ''}>
      <td><span className="cosmos-data-tooltip" data-tooltip={row.primary_message_type || undefined}><span className={`cosmos-tx-type cosmos-tx-type--${typeTone(row)}`}>{row.primary_action}{row.message_count > 1 && <small className="cosmos-tx-more">+{row.message_count - 1}</small>}</span></span></td>
      ...
      <td><time className="cosmos-data-tooltip" dateTime={row.timestamp} data-tooltip={row.timestamp}>{relativeTime(row.timestamp)}</time></td>
      ...
      <td>{fee(row, network.assets)}</td><td><span className="cosmos-data-tooltip cosmos-data-tooltip--right" data-tooltip={`Gas used: ${row.gas_used ?? '—'} · wanted: ${row.gas_wanted ?? '—'}`}>{compact(row.gas_used)}</span></td>
