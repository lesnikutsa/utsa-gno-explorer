...
function MarketPanel({ network, market, marketError, history }) {
  ...
  return <section className="panel cosmos-market"><div className="cosmos-market__summary"><div><span className="eyebrow">Market · ATONE</span><strong>{market ? `$${market.price}` : marketError ? 'Unavailable' : '—'}</strong><span className={`cosmos-market__change cosmos-market__change--${tone}`}>{market ? `${change > 0 ? '+' : ''}${market.change_24h}% (24h)` : 'Optional market enrichment'}</span></div><dl><Detail label="Market cap" value={market ? `$${market.market_cap}` : '—'} /><Detail label="Last update" value={market?.source_last_updated_at ? relativeTime(market.source_last_updated_at) : '—'} raw={market?.source_last_updated_at} /></dl></div>
    {path && <svg className={`cosmos-market__chart cosmos-market__chart--${tone}`} viewBox="0 0 300 80" role="img" aria-label={`${network.presentation.projectName} 24 hour USD price history`}><path className="cosmos-market__area" d={`${path} L300,80 L0,80 Z`} /><path className="cosmos-market__line" d={path} /></svg>}
  </section>
}

export function CosmosOverview({ network, overview: suppliedOverview, blocks: suppliedBlocks }) {
  ...
  return <CosmosOverviewView data={overview.data} market={market.data} marketError={market.error} history={history.data} blocks={blocks.data?.blocks || []} blocksError={blocks.error} stale={overview.stale} network={network} />
}

export function CosmosOverviewView({ data, market, marketError, history, blocks = [], blocksError, stale = false, network }) {
  ...
  const validatorColumns = [{ key: 'moniker', label: 'Validator', render: (row) => <CosmosValidatorIdentity moniker={row.moniker} address={row.operator_address || row.consensus_address} /> }, { key: 'missed_blocks_counter', label: 'Missed Blocks', render: (row) => <strong className="missed-value missed-value--medium">{row.missed_blocks_counter}</strong> }]
  ...
  return <div className="cosmos-overview">
    ...
    <div className="cosmos-parameters">
      ...
      <ParameterCard title="Node / Network"><dl className="cosmos-metrics"><Detail label="Tx index" value={data.network.tx_index} /><Detail label="Application" value={[data.network.application_name, data.network.application_version].filter(Boolean).join(' ') || '—'} /><Detail label="SDK version" value={data.network.sdk_version || '—'} /><Detail label="CometBFT" value={data.network.cometbft_version || '—'} /><Detail label="Node version" value={data.network.node_version || '—'} /><Detail label="Block history" value={data.network.block_history_state} /></dl></ParameterCard>
    </div>
  </div>
}
