...
export const CosmosSection = ({ title, data, children }) => <CosmosPanel><h2>{title}</h2>{isSectionError(data) ? <p className="section-error">This section is temporarily unavailable.</p> : children}</CosmosPanel>
export const CosmosMetric = ({ label, children, tip }) => <div className="cosmos-metric"><span>{label}{tip && <InfoTip>{tip}</InfoTip>}</span><strong>{children}</strong></div>
const Section = CosmosSection
const Metric = CosmosMetric

export function ValidatorMissesSection({ data }) {
  return <Section title="Active validators with most missed blocks" data={data}>{Array.isArray(data) && <><p className="section-note">This is the protocol counter within the slashing window, not Gno health over the last 1,000 blocks. Distance to threshold is not a guaranteed number of blocks before jail.</p>{data.length ? <div className="cosmos-validator-list">{data.map((validator) => <div key={validator.operator_address}><strong>{validator.moniker}</strong><span>{validator.missed_blocks_counter} missed · {validator.remaining_misses_before_threshold} to threshold</span><span>{validator.jailed ? 'Jailed' : 'Active'}{validator.tombstoned ? ' · Tombstoned' : ''}</span></div>)}</div> : <p>No active validator misses are reported.</p>}</>}</Section>
}
...
export function CosmosOverview({ network, onIdentity }) {
  ...
  return <div className="cosmos-page">
    ...
    <div className="cosmos-grid">
      <CosmosPanel><h2>Network</h2><Metric label="Status">{data.network.operational_state}</Metric><Metric label="Local RPC height">{value(data.network.current_local_height?.toLocaleString())}</Metric><Metric label="Last block">{date(data.network.latest_block_time)}</Metric></CosmosPanel>
      <CosmosPanel><h2>ATONE market</h2>{market.data ? <><Metric label="Price">${Number(market.data.price).toLocaleString()}</Metric><Metric label="24h change">{formatMarketPercent(market.data.change_24h)}</Metric><Metric label="Market cap">${Number(market.data.market_cap).toLocaleString()}</Metric><Metric label="CoinGecko updated">{date(market.data.source_last_updated_at)}</Metric></> : <p className="section-error">Market data is temporarily unavailable. Network data remains available.</p>}</CosmosPanel>
    ...
    <ValidatorMissesSection data={data.top_active_validators_by_missed_blocks} />
