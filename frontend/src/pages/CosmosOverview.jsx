...
export function CosmosOverviewView({ data, market, marketError, history, blocks = [], blocksError, blocksStale = false, stale = false, network, averageBlockSeconds }) {
  ...
  const validatorColumns = [{ key: 'moniker', label: 'Validator', render: (row) => <CosmosValidatorIdentity moniker={row.moniker} address={row.operator_address || row.consensus_address} /> }, { key: 'missed_blocks_counter', label: 'Liveness risk', render: (row) => { const risk = riskFor(row); return <span className="cosmos-risk"><strong>{row.missed_blocks_counter.toLocaleString()} missed</strong>{risk && <><span>Miss budget: {risk.budgetLeft.toLocaleString()} left</span><span>Earliest penalty: {risk.overThreshold ? 'threshold exceeded' : formatApproximateDuration(risk.seconds)}</span><span className={`cosmos-risk__bar cosmos-risk__bar--${risk.tone}`}><i style={{ width: `${risk.usage * 100}%` }} /></span></>}<ParameterHelp label="earliest downtime penalty">Estimate assumes future missed blocks continue increasing the rolling missed-block counter. Exact penalty time may change as older misses leave the signing window.</ParameterHelp></span> } }]
