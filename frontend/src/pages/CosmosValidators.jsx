...
export function CosmosValidators({ network }) {
  ...
  return <section className="cosmos-validators">
    <header className="cosmos-validators__heading"><h1>Validators</h1><InfoTooltip label="About Cosmos validators" content="Voting power, approximate 24h movement, and live signing risk." /></header>
    <div className="cosmos-validator-summary">
      ...
      <article><span>24h bonded change <InfoTooltip label="About 24h bonded change" content="Approximate 24h change derived from current and historical consensus voting power." /></span><strong>{data.summary.bonded_change_24h == null ? 'History unavailable' : `${signed(fmt(data.summary.bonded_change_24h, asset.exponent), 2)} ${asset.symbol}`}</strong></article>
    </div>
...
function Liveness({ validator, state }) { const live = validator.liveness; if (!live) return <span className="muted">Liveness unavailable</span>; const strip = validator.signing_strip || []; const usage = live.allowed_misses ? live.missed_blocks / live.allowed_misses : 1; const tone = cosmosRiskToneFromUsage(usage); return <div className="validator-liveness"><div><strong className={missedCountClass(live.missed_blocks)}>{live.missed_blocks.toLocaleString()} missed</strong> · {live.signed_percent.toFixed(2)}% signed</div><small>Budget left: {live.remaining_budget.toLocaleString()} · Jail ETA: <span>{duration(live.jail_eta_seconds)} <InfoTooltip label="About Jail ETA" content="Estimated time until the missed-block threshold if the validator continues missing every block." /></span></small><div className={`validator-budget cosmos-risk__bar cosmos-risk__bar--${tone}`}><i style={{ width: `${Math.min(100, usage * 100)}%` }} /></div>{strip.length ? <div className="validator-signing-strip" aria-label="Recent 50-block signing history">{strip.map((point) => <InfoTooltip key={point.height} label={`Block ${point.height} ${point.status}`} content={<><strong>Block #{point.height}</strong><span>{point.status[0].toUpperCase() + point.status.slice(1)}</span>{pointTime(point) && <span>{pointTime(point)}</span>}</>}><i className={`is-${point.status}`} /></InfoTooltip>)}</div> : <small className="muted">{state === 'warming' ? 'Loading recent signing history…' : 'Recent history unavailable'}</small>}</div> }
