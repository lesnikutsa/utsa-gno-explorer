...
import { directedValidatorComparison, favoriteFirst, missedCountClass, validatorRankTone } from '../utils/cosmosValidators'

const TABS = ['active', 'inactive', 'jailed']
const fmt = (raw, exponent = 0) => Number(raw || 0) / 10 ** exponent
const wholeTokens = (raw, exponent = 0) => Math.trunc(fmt(raw, exponent))
...
export function CosmosValidators({ network }) {
  ...
  const bonded = wholeTokens(data.summary.bonded_tokens, asset.exponent)
  return <section className="cosmos-validators">
    ...
    <div className="cosmos-validator-summary">
      <article className="card status-card cosmos-validator-summary__card"><span>Active validators</span><strong>{data.summary.active_validators}</strong></article>
      <article className="card status-card cosmos-validator-summary__card"><span>Bonded tokens</span><strong>{bonded.toLocaleString()} {asset.symbol}</strong></article>
      <article className="card status-card cosmos-validator-summary__card"><span>Bonded ratio</span><strong>{data.summary.bonded_ratio == null ? '—' : `${(data.summary.bonded_ratio * 100).toFixed(2)}%`}</strong></article>
      <article className="card status-card cosmos-validator-summary__card"><span>24h bonded change</span><strong>{data.summary.bonded_change_24h == null ? 'History unavailable' : `${signed(wholeTokens(data.summary.bonded_change_24h, asset.exponent))} ${asset.symbol}`}</strong></article>
    </div>
    <div className="cosmos-validator-toolbar"><div className="cosmos-validator-tabs" role="tablist">{TABS.map((kind) => <button key={kind} role="tab" aria-selected={tab === kind} className={tab === kind ? 'is-active' : ''} onClick={() => setTab(kind)}>{kind[0].toUpperCase() + kind.slice(1)} <b>{counts[kind]}</b></button>)}</div><div className="cosmos-validator-search"><input type="search" aria-label="Search validators" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search moniker or valoper address" />{query && <button className="cosmos-validator-search__clear" type="button" onClick={() => setQuery('')} aria-label="Clear validator search">×</button>}</div></div>
    ...
    <tbody>{rows.map((validator) => <tr key={validator.operator_address}><td><span className={`cosmos-validator-rank cosmos-validator-rank--${validatorRankTone(validator.stake_share)}`}>{powerRanks.get(validator.operator_address)}</span></td><td><span className="cosmos-validator-favorite-identity"><button className={`validator-favorite ${favorites.has(validator.operator_address) ? 'validator-favorite--active' : ''}`} type="button" aria-pressed={favorites.has(validator.operator_address)} aria-label={`${favorites.has(validator.operator_address) ? 'Remove' : 'Add'} ${validator.moniker} ${favorites.has(validator.operator_address) ? 'from' : 'to'} favorites`} onClick={() => toggleFavorite(validator.operator_address)}>{favorites.has(validator.operator_address) ? '★' : '☆'}</button><a className="cosmos-validator-identity-link" href={`/networks/${network.id}/validators/${encodeURIComponent(validator.operator_address)}`}><CosmosValidatorIdentity moniker={validator.moniker} address={validator.operator_address} imageSrc={validator.avatar_url} showTitles={false} /></a></span></td><td><strong className="cosmos-validator-primary-metric">{wholeTokens(validator.tokens, asset.exponent).toLocaleString()} {asset.symbol}</strong><small className="cosmos-validator-stake-share">{validator.stake_share.toFixed(2)}%</small></td>{tab === 'active' && <td><Delta validator={validator} asset={asset} /></td>}{tab === 'jailed' && <><td>{jailedUntil(validator.jailed_until)}</td><td>{validator.tombstoned == null ? '—' : validator.tombstoned ? 'Yes' : 'No'}</td></>}<td><strong className="cosmos-validator-primary-metric">{(Number(validator.commission) * 100).toFixed(2)}%</strong></td>{tab === 'active' && <td><Liveness validator={validator} state={data.signing_history_state} /></td>}</tr>)}</tbody></table></div>
  </section>
}
function Delta({ validator, asset }) { if (validator.change_24h == null) return <span className="muted">History unavailable</span>; if (validator.change_24h === '0') return <span className="muted">—</span>; const preciseValue = fmt(validator.change_24h, asset.exponent); const value = Math.trunc(preciseValue); const tone = preciseValue > 0 ? 'positive' : preciseValue < 0 ? 'negative' : 'neutral'; return <span className={`validator-delta is-${tone}`}><strong>{signed(value)} {asset.symbol}</strong><small>{validator.change_24h_percent == null ? '—' : `${signed(validator.change_24h_percent, 2)}%`}</small></span> }
