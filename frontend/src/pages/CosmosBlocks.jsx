...
export function CosmosBlocks({ network, resource }) {
  ...
  return <div className="cosmos-blocks"><div className="cosmos-title"><h1>Blocks</h1>{resource.stale
    ...
    {resource.data && <section className="panel cosmos-blocks-table"><div className="cosmos-table"><table><thead><tr><th>Height</th><th>Time</th><th>Proposer</th><th>Txs</th><th>Block hash</th></tr></thead><tbody>{rows.length ? rows.map((block, index) => <tr key={block.height} className={index === 0 && block.height === newest ? 'is-new-row' : index < 3 ? 'is-settling-row' : ''}>
      ...
      <td><time className="cosmos-data-tooltip" dateTime={block.timestamp} data-tooltip={block.timestamp}>{relativeTime(block.timestamp)}</time></td>
      <td><CosmosValidatorIdentity moniker={block.proposer_moniker} address={block.proposer_operator_address || block.proposer} imageSrc={block.proposer_avatar_url} showTitles={false} href={block.proposer_operator_address ? `/networks/${network.id}/validators/${encodeURIComponent(block.proposer_operator_address)}` : undefined} /></td>
      <td>{block.transaction_count.toLocaleString()}</td><td><code className="muted cosmos-data-tooltip cosmos-data-tooltip--right" data-tooltip={block.hash}>{shortHash(block.hash)}</code></td>
