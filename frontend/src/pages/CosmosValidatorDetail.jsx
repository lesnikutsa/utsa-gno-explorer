...
function Address({ label, value, full = false, accent = false }) { const display = value || '—'; return <div><dt>{label}</dt><dd className={`cosmos-copy-value cosmos-validator-address${full ? ' is-full' : ''}${accent ? ' is-accent' : ''}`}><code>{display}</code>{value && <CopyButton value={value} label={label.toLowerCase()} />}</dd></div> }
...
function DelegatorShares({ value }) { if (value == null) return '—'; return <span className="cosmos-validator-shares" title={`Raw shares: ${value}`}><span>{formatCompactDecimal(String(value), { digits: 2 })} shares</span><CopyButton value={String(value)} label="delegator shares" /></span> }
function RewardField({ label: rewardLabel, coins, assets }) { return <Field label={rewardLabel} value={<RewardList coins={coins} assets={assets} />} /> }
function RewardList({ coins, assets }) {
  const [expanded, setExpanded] = useState(false)
  if (!coins?.length) return '—'
  const visible = expanded ? coins : coins.slice(0, 4)
  const remaining = Math.max(0, coins.length - 4)
  return <div className="cosmos-validator-rewards">{visible.map((coin) => <span key={`${coin.denom}:${coin.amount}`}>{formatRewardCoin(coin, assets)}</span>)}{remaining > 0 && <button type="button" className="cosmos-validator-rewards__toggle" onClick={() => setExpanded((value) => !value)}>{expanded ? 'Show less ↑' : `Show ${remaining} more ↓`}</button>}</div>
}
