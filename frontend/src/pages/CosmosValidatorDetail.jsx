...
function Panel({ title, children, className = '' }) { return <section className={`panel cosmos-validator-fields ${className}`.trim()}><h2>{title}</h2><dl>{children}</dl></section> }
...
function Delegators({ network, operatorAddress, assets, totalShares, validatorAccountAddress }) {
  ...
  const ordered = useMemo(() => sort === 'delegated' ? [...state.items].sort((a, b) => { const left = BigInt(a.balance.amount); const right = BigInt(b.balance.amount); return left === right ? 0 : left > right ? -1 : 1 }) : state.items, [sort, state.items])
