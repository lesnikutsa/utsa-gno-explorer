...
export function CosmosValidatorDetail({ network, operatorAddress }) {
  ...
  return <section className="cosmos-validator-detail theme-compatible">
    ...
    <header className="panel cosmos-validator-hero">
      ...
      <div className="cosmos-validator-hero__facts"><Field label="Identity" value={v.identity || '—'} /><Field label="Status" value={label(v.bond_status)} /><Field label="Jailed" value={v.jailed ? 'Yes' : 'No'} /><Field label="Rank" value={<span className={`cosmos-validator-rank cosmos-validator-rank--${validatorRankTone(v.stake_share)}`}>#{v.rank}</span>} /></div>
      {site && <a className="cosmos-validator-website" href={site} target="_blank" rel="noopener noreferrer">Visit website ↗</a>}{v.description && <p>{v.description}</p>}{v.jailed && <div className="cosmos-validator-jailed"><strong>Jailed</strong><span>Until: {utc(v.jailed_until)}</span>{v.tombstoned != null && <span>Tombstoned: {v.tombstoned ? 'Yes' : 'No'}</span>}</div>}
    </header>
    ...
    <div className="cosmos-validator-detail__secondary"><Panel title="Validator Parameters"><Field label="Bonded Tokens" value={formatTokenAmount(v.tokens, asset.exponent, asset.symbol)} /><Field label="Delegator Shares" value={readableDecimal(v.delegator_shares)} />{v.min_self_delegation != null && <Field label="Minimum Self Delegation" value={v.min_self_delegation} />}<Field label="Commission Rate" value={pct(v.commission.rate)} />{v.commission.max_rate != null && <Field label="Max Commission" value={pct(v.commission.max_rate)} />}{v.commission.max_change_rate != null && <Field label="Max Daily Change" value={pct(v.commission.max_change_rate)} />}{v.commission.update_time && <Field label="Commission Updated" value={utc(v.commission.update_time)} />}</Panel><Panel title="Consensus Identity"><Address label="Account Address" value={v.account_address} /><Address label="Operator Address" value={v.operator_address} /><Address label="Consensus Address (valcons)" value={v.consensus_address} />{v.consensus_pubkey && <Address label="Consensus Public Key" value={v.consensus_pubkey} full />}<Address label="Hex Address" value={v.hex_address} />{v.evm_address && <Address label="EVM Address" value={v.evm_address} />}</Panel></div>
  </section>
}
...
function Address({ label, value, full = false }) { return <div><dt>{label}</dt><dd className={`cosmos-copy-value cosmos-validator-address${full ? ' is-full' : ''}`}><code>{value}</code><CopyButton value={value} label={label.toLowerCase()} /></dd></div> }
