...
function Address({ label, value, full = false, href, accent = false }) { const display = value || '—'; return <div><dt>{label}</dt><dd className={`cosmos-copy-value cosmos-validator-address${full ? ' is-full' : ''}${accent ? ' is-accent' : ''}`}>{value && href ? <a href={href}><code>{display}</code></a> : <code>{display}</code>}{value && <CopyButton value={value} label={label.toLowerCase()} />}</dd></div> }
