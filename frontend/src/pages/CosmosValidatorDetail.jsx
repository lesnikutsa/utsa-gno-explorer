  ...
  return <section className="cosmos-validator-detail theme-compatible">
    ...
    <header className="panel cosmos-validator-hero">
      <div className="cosmos-validator-hero__profile">
        ...
        {(site || v.contact) && <div className="cosmos-validator-hero__links">{site && <a className="cosmos-validator-website" href={site} target="_blank" rel="noopener noreferrer">Website ↗</a>}{v.contact && <span><b>Contact</b> {v.contact}</span>}</div>}
      </div>
      <div className="cosmos-validator-hero__facts"><Field label="Status" value={label(v.bond_status)} /><Field label="Jailed" value={v.jailed ? 'Yes' : 'No'} /><Field label="Rank" value={<span className={`cosmos-validator-rank cosmos-validator-rank--${validatorRankTone(v.stake_share)}`}>#{v.rank}</span>} /><Field label="Minimum Self Delegation" value={v.min_self_delegation == null ? '—' : formatTokenAmount(v.min_self_delegation, asset.exponent, asset.symbol)} /></div>
      {v.identity && <div className="cosmos-validator-hero__identity"><span>Identity</span><code>{v.identity}</code></div>}
