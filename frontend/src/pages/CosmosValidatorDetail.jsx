...
function Delta({ validator: v, asset }) { if (v.change_24h == null || Number(v.change_24h) === 0) return '—'; const positive = Number(v.change_24h) > 0; return <span className={`validator-delta is-${positive ? 'positive' : 'negative'}`}>{formatSignedTokenAmount(v.change_24h, asset.exponent, asset.symbol)}{v.change_24h_percent != null && <small>{v.change_24h_percent > 0 ? '+' : ''}{v.change_24h_percent.toFixed(2)}%</small>}</span> }
