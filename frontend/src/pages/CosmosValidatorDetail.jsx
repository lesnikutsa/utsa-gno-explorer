...
function ValidatorActivity({ network, operatorAddress, assets }) {
  ...
  return <section className="panel cosmos-validator-activity"><div className="panel__heading"><div><h2>Validator Activity</h2><span className="panel__meta">Recent validator-related transactions{state.status === 'partial' && <ParameterHelp label="validator activity availability">Some validator activity types are unavailable from the current RPC/API.</ParameterHelp>}</span></div></div>{state.status !== 'indexing_unavailable' && <div className="cosmos-validator-activity__filters" aria-label="Filter loaded validator activity">{Object.keys(activityGroups).map((key) => <button key={key} type="button" className={filter === key ? 'is-active' : ''} aria-pressed={filter === key} onClick={() => setFilter(key)}>{label(key)}</button>)}</div>}{state.status === 'indexing_unavailable' ? <p className="muted">Validator activity is unavailable from the current RPC/API.</p> : <>{state.status === 'error' && !state.items.length ? <p className="cosmos-error">Validator activity is temporarily unavailable.</p> : state.status === 'loading' ? <p className="muted">Loading validator activity…</p> : !rows.length ? <p className="muted">No recent validator activity found.</p> : <div className="cosmos-validator-activity__scroll"><table><thead><tr><th>Activity</th><th>Amount / detail</th><th>Height / time</th><th>TX</th></tr></thead><tbody>{rows.map((item) => <tr key={`${item.tx_hash}:${item.message_index}:${item.action}`}><td><strong className={`is-${item.direction}`}>{activityLabel(item.action)}</strong><code title={item.account_address || ''}>{shortValue(item.account_address)}</code></td><td className={`is-${item.direction}`}>{item.amounts.length ? item.amounts.map((coin) => <span key={coin.denom}>{item.direction === 'positive' ? '+' : item.direction === 'negative' ? '−' : ''}{formatDelegationBalance(coin, assets)}</span>) : item.detail || '—'}</td><td><a href={`/networks/${network.id}/blocks/${item.height}`}>#{item.height}</a><time>{utc(item.timestamp)}</time></td><td><a href="#" className="mono cosmos-validator-activity__tx" title={item.tx_hash} onClick={(event) => openTx(event, item.tx_hash)}>{shortValue(item.tx_hash)}</a></td></tr>)}</tbody></table></div>}<div className="cosmos-validator-delegators__actions">{state.expanded && filteredItems.length > 10 && <button className="cosmos-detail-toggle" type="button" onClick={() => setState((current) => ({ ...current, expanded: false }))}>Show less ↑</button>}{hiddenLoadedRows ? <button className="cosmos-detail-toggle" type="button" onClick={() => setState((current) => ({ ...current, expanded: true }))}>Show 10 more ↓</button> : state.hasMore && <button className="cosmos-detail-toggle" type="button" disabled={state.loadingMore} onClick={more}>{state.loadingMore ? 'Loading activity…' : 'Show 10 more ↓'}</button>}</div></>}</section>
}
