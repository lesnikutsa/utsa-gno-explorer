...
export function CosmosValidatorDetail({ network, operatorAddress }) {
  ...
  return <section className="cosmos-validator-detail theme-compatible">
    ...
    <Delegators network={network} operatorAddress={v.operator_address} assets={registeredAssets} totalShares={v.delegator_shares} />
  </section>
}
...
function Delegators({ network, operatorAddress, assets, totalShares }) {
  ...
  return <section className="panel cosmos-validator-delegators"><div className="panel__heading"><div><h2>Delegators</h2><span className="panel__meta">Current delegations to this validator</span></div></div>
    {state.loading ? <p className="muted">Loading delegators…</p> : state.error && !state.items.length ? <p className="cosmos-error">Delegator data is temporarily unavailable.</p> : !state.items.length ? <p className="muted">No delegations found.</p> : <><div className="cosmos-validator-delegators__scroll"><table><thead><tr><th>Delegator</th><th>Delegated</th><th title="Share of this validator's total delegator shares.">Share</th></tr></thead><tbody>{visible.map((item) => <tr key={`${item.delegator_address}:${item.validator_address}`}><td><span className="cosmos-validator-delegator"><code>{item.delegator_address}</code><CopyButton value={item.delegator_address} label="delegator address" /></span></td><td className="cosmos-validator-delegated" title={`${item.balance.amount} ${item.balance.denom}`}>{formatDelegationBalance(item.balance, assets)}</td><td className="cosmos-validator-delegation-share" title={`Exact shares: ${item.shares}`}>{formatDelegationShare(item.shares, totalShares)}</td></tr>)}</tbody></table></div><div className="cosmos-validator-delegators__actions">{state.expanded && state.items.length > 10 && <button type="button" onClick={() => setState((current) => ({ ...current, expanded: false }))}>▾ Show less</button>}{state.expanded && state.nextKey && <button type="button" disabled={state.loadingMore} onClick={showMore}>{state.loadingMore ? 'Loading delegators…' : '▸ Show 10 more'}</button>}{!state.expanded && state.items.length > 10 && <button type="button" onClick={() => setState((current) => ({ ...current, expanded: true }))}>▸ Show 10 more</button>}{!state.expanded && state.items.length <= 10 && state.nextKey && <button type="button" onClick={showMore}>▸ Show 10 more</button>}</div>{state.error && <p className="cosmos-error">Delegator data is temporarily unavailable.</p>}</>}
  </section>
}
