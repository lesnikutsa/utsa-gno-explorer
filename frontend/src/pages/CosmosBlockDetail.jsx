...
import '../styles/cosmos-copy-controls.css'
...
const Hash = ({ value, label, compact = false }) => value ? <span className="cosmos-copy-value"><code className={compact ? undefined : 'cosmos-hash-value'}>{value}</code><CopyButton value={value} label={`Copy ${label}`} showTitle={false} /></span> : <span>—</span>
...
function AvailableBlock({ network, lookup, height }) {
  ...
  return <><nav className="cosmos-block-nav">{data.height > 1 && <a href={`/networks/${network.id}/blocks/${data.height - 1}`}>← #{(data.height - 1).toLocaleString()}</a>}<span />{data.height < lookup.local_height ? <a href={`/networks/${network.id}/blocks/${data.height + 1}`}>#{(data.height + 1).toLocaleString()} →</a> : <span>Latest</span>}</nav>
    <section className="cosmos-detail-summary panel"><Metric label="Height">{data.height.toLocaleString()}</Metric><Metric label="Time"><time>{relativeTime(data.timestamp)}</time><small>{utc(data.timestamp)}</small></Metric><Metric label="Transactions">{data.transaction_count.toLocaleString()}</Metric><Metric label="Chain ID">{data.chain_id}</Metric></section>
    <section className="panel cosmos-detail-card"><h2>Block Information</h2><dl><dt>Block hash</dt><dd><Hash value={data.hashes.block} label="block hash" /></dd><dt>Proposer</dt><dd><span className="cosmos-copy-value"><CosmosValidatorIdentity moniker={data.proposer_moniker} address={data.proposer_operator_address || data.proposer} imageSrc={data.proposer_operator_address ? avatarByOperator.get(data.proposer_operator_address) : undefined} showTitles={false} href={data.proposer_operator_address ? `/networks/${network.id}/validators/${encodeURIComponent(data.proposer_operator_address)}` : undefined} /><CopyButton value={data.proposer_operator_address || data.proposer} label="Copy proposer address" showTitle={false} /></span></dd><dt>Timestamp</dt><dd><time>{utc(data.timestamp)} · {relativeTime(data.timestamp)}</time></dd><dt>Block version</dt><dd>{data.block_version}</dd><dt>App version</dt><dd>{data.app_version}</dd></dl></section>
