...
test('signing points render exact block context without another request', () => {
  assert.match(source, /<strong>Block #\{point\.height\}<\/strong>/)
  assert.match(source, /point\.status/)
  assert.match(source, /pointTime\(point\)/)
  assert.match(source, /<InfoTooltip key=\{point\.height\}/)
...
test('custom tooltips, zero delta, compact search, and jailed layout replace native UI', () => {
  assert.match(source, /InfoTooltip label="About Cosmos validators"/)
  assert.match(source, /InfoTooltip label="About 24h validator change"/)
  assert.match(source, /InfoTooltip label="About Jail ETA"/)
  ...
  assert.match(css, /\.info-tooltip__popover/)
})
