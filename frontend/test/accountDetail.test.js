...
test('transaction cards distinguish linked values from plain information', () => {
  const transactions = page.slice(page.indexOf('function AccountTransactions'), page.indexOf('function MissingAccount'))
  assert.ok(transactions.includes('className="account-detail__transaction-operation" href={`/blocks/${item.block_height}/transactions/${item.index}`}'))
  assert.ok(transactions.includes('className={`account-detail__direction'))
  assert.equal(/<a[^>]*>\{direction\}<\/a>/.test(transactions), false)
  assert.ok(transactions.includes('<span className="account-detail__transaction-amount">{String(item.amount)}</span>'))
...
test('transaction metadata is labeled, accessible, and ready for a future status', () => {
  const transactions = page.slice(page.indexOf('function AccountTransactions'), page.indexOf('function MissingAccount'))
  for (const label of ['>Block</dt>', '>Time</dt>', '>Tx hash</dt>']) assert.ok(transactions.includes(label))
  assert.ok(transactions.includes('href={`/blocks/${item.block_height}`}'))
  assert.ok(transactions.includes('<time dateTime={item.block_time}>'))
  ...
  assert.ok(transactions.includes('className="account-detail__transaction-status-slot" aria-hidden="true"'))
  for (const status of ['Success', 'Failed', 'Pending', 'Unknown']) assert.equal(transactions.includes(status), false)
})
test('transaction card CSS provides compact desktop and responsive layouts', () => {
  const transactionStyles = styles.slice(styles.indexOf('.account-detail__transaction-list'))
  assert.ok(transactionStyles.includes('grid-template-columns: minmax(0, 1fr) minmax(220px, auto)'))
  assert.ok(transactionStyles.includes('.account-detail__transaction-aside { display: grid;'))
  assert.ok(transactionStyles.includes('justify-items: end'))
  assert.ok(transactionStyles.includes('.account-detail__transaction-meta-value {'))
  assert.ok(transactionStyles.includes('font-size: 11px; text-align: right'))
  ...
  for (const className of ['account-detail__transaction-counterparty', 'account-detail__transaction-hash', 'account-detail__transaction-meta-label']) assert.ok(transactionStyles.includes(`.${className}`))
  assert.match(transactionStyles, /@media \(max-width: 760px\)[\s\S]*?account-detail__transaction \{ grid-template-columns: minmax\(0, 1fr\); \}/)
  assert.match(transactionStyles, /account-detail__transaction-aside \{ grid-template-rows: auto; justify-items: start; \}/)
  assert.match(transactionStyles, /account-detail__transaction-meta-value \{ text-align: left; \}/)
