...
test('page uses a compact balance and account summary overview', () => {
  ...
  for (const value of ['Account number', 'Sequence']) assert.ok(accountSummary.includes(value))
  for (const value of ['Data source', 'RPC', 'Chain ID', 'Fetched at block', 'observed_height']) assert.equal(accountSummary.includes(value), false)
})
...
test('validator relation is a compact message without exposed addresses', () => {
  const validatorCard = page.slice(page.indexOf('{account.validator_relation &&'), page.indexOf('<TransactionsPlaceholder />', page.indexOf('{account.validator_relation &&')))
  ...
  assert.ok(validatorCard.includes('View validator →'))
