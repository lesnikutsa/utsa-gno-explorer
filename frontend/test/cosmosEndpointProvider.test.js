...
test('endpoint popup stays compact while exposing Auto and paired manual mode', () => {
  ...
  assert.match(component, /\{height\(provider\.rpc\.height\)\}/)
  assert.match(component, /R \{compactLatency\(provider\.rpc\.latency_ms\)\} · A \{compactLatency\(provider\.api\.latency_ms\)\} ms/)
  assert.match(component, /cosmos-endpoint-mode__dot/)
  ...
  assert.match(styles, /\.rpc-pool__row\.cosmos-endpoint-mode__health \{/)
