...
test('detail presents validator-only identity, metrics, signing, slashing and parameters', () => {
  ...
  assert.match(css, /\.cosmos-validator-hero__metrics article\s*\{[^}]*border:\s*1px solid var\(--color-border-soft\)[^}]*border-radius:\s*6px/)
  assert.match(css, /\.cosmos-validator-hero__metrics article\s*\{[^}]*box-shadow:\s*inset 0 0 0 1px var\(--color-card\)/)
