...
test('detail presents validator-only identity, metrics, signing, slashing and parameters', () => {
  for (const text of ['Contact', 'Minimum Self Delegation', 'Account Address', 'Hex Address', 'Jailed', 'Identity', 'Voting Power', 'Stake Share', '≈24h Change', 'Commission', 'Recent finalized participation', 'Protocol slashing window', 'Rewards & Commission', 'Validator commission earned', 'Delegators total rewards', 'Consensus Identity', 'Consensus Public Key']) assert.match(page, new RegExp(text))
  ...
  assert.match(css, /\.cosmos-validator-signing__monitor\s*\{[^}]*width:\s*100%[^}]*margin:\s*0/)
  ...
  assert.match(page, /Address label="Account Address"[^>]+href=\{`\/networks\/\$\{network\.id\}\/validators\?category=\$\{v\.category\}&search=/)
  assert.match(list, /TABS\.includes\(listParams\(\)\.get\('category'\)\)/)
  assert.match(css, /\.cosmos-validator-address\.is-accent a code\s*\{\s*color:\s*var\(--color-accent\)/)
