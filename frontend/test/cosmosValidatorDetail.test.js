...
test('detail presents validator-only identity, metrics, signing, slashing and economics', () => {
  for (const text of ['Contact', 'Minimum Self Delegation', 'Account Address', 'Consensus Hex Address', 'Jailed', 'Voting Power', 'Stake Share', '≈24h Change', 'Commission', 'Recent finalized participation', 'Protocol slashing window', 'Rewards & Commission', 'Validator Commission', 'Outstanding Rewards', 'Consensus Identity', 'Consensus Public Key', 'Delegator Shares']) assert.match(page, new RegExp(text))
  ...
  assert.match(detailCss, /grid-template-columns:\s*repeat\(25, 11px\)/)
  assert.match(detailCss, /span\.is-latest/)
  assert.match(detailCss, /grid-template-columns:\s*minmax\(0, \.8fr\) minmax\(0, 1fr\)/)
  ...
  assert.match(page, /formatCompactDecimal\(String\(value\), \{ digits: 2 \}\)/)
  ...
  assert.match(page, /coins\.slice\(0, 4\)/)
