...
test('Cosmos node strip stays on one row throughout collapsed browser widths and degrades only for mobile', () => {
  assert.match(styles, /@media \(max-width: 1100px\) and \(min-width: 761px\)[\s\S]*grid-template-columns:\s*\.62fr 1\.16fr \.72fr \.78fr \.82fr 1\.52fr \.86fr;/)
  assert.doesNotMatch(styles, /@media \(max-width: 899px\) and \(min-width: 761px\)/)
