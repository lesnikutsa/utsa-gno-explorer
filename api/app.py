...
from api.cosmos.schemas import (
    ...
    BlockLookupResponse as CosmosBlockLookupResponse,
    BlocksResponse as CosmosBlocksResponse,
    MarketHistoryResponse as CosmosMarketHistoryResponse,
    MarketResponse as CosmosMarketResponse,
...
@app.get("/api/networks", response_model=CosmosPublicNetworksResponse)
async def get_public_cosmos_networks():
    return {"networks": public_networks()}


@app.get("/api/networks/{network_id}/overview", response_model=CosmosOverviewResponse)
...
async def get_cosmos_validator_detail(network_id: str, operator_address: str):
    service = _cosmos_service(network_id)
    try:
        return await service.validator_detail(operator_address)
        ...
        raise HTTPException(status_code=404, detail="Validator not found") from None
    except Exception:
        LOGGER.exception("Cosmos validator detail failed network=%s", network_id)
        raise HTTPException(status_code=503, detail="Validator data is temporarily unavailable") from None


@app.get("/api/networks/{network_id}/validators/{operator_address}/delegations",
         response_model=CosmosValidatorDelegationsResponse, response_model_exclude_none=True)
async def get_cosmos_validator_delegations(
        network_id: str, operator_address: str,
        limit: int = Query(default=10, ge=1, le=20),
        pagination_key: str | None = Query(default=None, min_length=1, max_length=512)):
    service = _cosmos_service(network_id)
    try:
        return await service.validator_delegations(operator_address, limit, pagination_key)
    except InvalidValidatorAddress:
        raise HTTPException(status_code=400, detail="Invalid validator operator address") from None
    except ValueError:
        raise HTTPException(status_code=422, detail="Invalid delegation pagination") from None
    except Exception:
...
@app.get("/api/networks/{network_id}/market/history", response_model=CosmosMarketHistoryResponse)
...
@app.get("/api/networks/{network_id}/transactions", response_model=CosmosTransactionsResponse)
async def get_cosmos_transactions(network_id: str, limit: int = Query(20, ge=1, le=20),
                                  page: int = Query(1, ge=1, le=100)):
    service = _cosmos_service(network_id)
    try:
        return await service.transactions(limit, page)
    except Exception:
...
@app.get("/api/networks/{network_id}/blocks/{height}/detail", response_model=CosmosBlockDetailResponse,
         response_model_exclude_none=True)
...
@app.get("/api/networks/{network_id}/blocks/{height}/transactions/{index}",
         response_model=CosmosTransactionDetailResponse)
