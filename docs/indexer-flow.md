...
- Store `vote_status = 'commit'` and `signed = true` only when the validator has a non-null precommit whose `Vote.BlockID` matches the enclosing `Commit.BlockID` and has a usable base64 Ed25519 signature that decodes to exactly 64 bytes.
