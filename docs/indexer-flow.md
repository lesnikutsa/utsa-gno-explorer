...
Block, commit, and validators requests for one endpoint and one height remain concurrent. Heights and PostgreSQL writes remain sequential. An endpoint-specific RPC, transport, malformed-payload, anchor, or parent-continuity failure removes that candidate for the current height and immediately tries the next safe URL at the same height. An endpoint proves the current checkpoint once when it is activated; continued use relies on each new header parent and does not refetch the previous block. A replacement proves the latest committed checkpoint once before fetching the failed height. Database errors, chain identity errors, finalized-data conflicts, checkpoint sequence violations, advisory-lock loss, and unclassified programming errors remain fatal and are never converted into failover.
