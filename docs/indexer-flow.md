...
- Compare each parsed `Vote.BlockID` with the enclosing `Commit.BlockID` for height `S`, including hash, part-set total, and part-set hash. A non-null signature alone is not sufficient for signing.
- Store `vote_status = 'commit'` and `signed = true` only when the validator has a non-null precommit whose `Vote.BlockID` matches the enclosing `Commit.BlockID` and has a usable non-empty base64 signature.
