...
The Realm Calls API depends on least-privilege PostgreSQL grants from migration `0010_grant_realm_calls_api_columns.sql`. The API role reads only the block time, transaction position/hash, and indexer checkpoint columns required by the response; raw block and transaction payload columns remain inaccessible to the API role.
