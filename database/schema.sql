...
CREATE TABLE realm_metadata (
 ...
 CONSTRAINT realm_metadata_counts_check CHECK (file_count BETWEEN 0 AND 256 AND gno_file_count BETWEEN 0 AND file_count AND test_file_count BETWEEN 0 AND gno_file_count AND total_file_bytes BETWEEN 0 AND 8388608 AND total_file_lines >= 0 AND dependency_count >= 0),
 ...
 CONSTRAINT realm_metadata_qrender_check CHECK ((qrender_sha256 IS NULL OR qrender_sha256 ~ '^[0-9a-f]{64}$') AND (qrender_byte_count IS NULL OR qrender_byte_count >= 0) AND (qrender_line_count IS NULL OR qrender_line_count >= 0)),
 CONSTRAINT realm_metadata_qstorage_check CHECK ((qstorage_bytes IS NULL OR qstorage_bytes >= 0) AND (qstorage_deposit_ugnot IS NULL OR qstorage_deposit_ugnot >= 0)),
 CONSTRAINT realm_metadata_package_capabilities_check CHECK (path_kind <> 'package' OR (qrender_status='not_applicable' AND qstorage_status='not_applicable'))
);
