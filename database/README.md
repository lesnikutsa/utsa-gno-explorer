...
### Applying migration 0006 safely
...
## GRC20 token directory

`GET /api/tokens` derives its directory from existing Realm catalog and metadata rows; there is no token whitelist or token table. A Realm is confirmed only when it has the exact `gno.land/p/demo/tokens/grc20` persisted import and its persisted `qfuncs` exposes `TotalSupply`, `BalanceOf`, and `Transfer`. Identity is best-effort and is published only for one unambiguous literal `NewToken` constructor in bounded persisted `.gno` source. Current supply, prices, holders, and balances are intentionally not indexed. Activity uses the existing Realm catalog counters and the indexed checkpoint timestamp.
