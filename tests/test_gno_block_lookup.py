...
def status(height=2_000, chain=CHAIN, timestamp=LATEST.isoformat()):
    return {"result": {"node_info": {"network": chain}, "sync_info": {
        "latest_block_height": str(height), "latest_block_time": timestamp,
        "catching_up": False}}}
...
class Client:
    def __init__(self, available=(1000, 1500, 1800, 1920), seconds=5):
...
def test_sparse_checkpoint_preference_and_fallbacks(available, span):
    result = run(2_100, Client(available))
...
def test_distant_future_height_has_bounded_valid_eta():
    result = run(1_000_000, Client((1000,)))
...
@pytest.mark.parametrize("payload", [status(chain="wrong-chain"), status(timestamp="bad")])
def test_wrong_chain_and_malformed_status_are_rejected(payload):
    with pytest.raises(Exception):
        run(2_100, Client(), payload=payload)
