...
class TransactionsFrontendContractTests(unittest.TestCase):
    ...
    def test_states_pagination_and_responsive_layout(self):
        ...
        self.assertIn("min-width: 1042px", transactions_rules)
        self.assertIn("grid-template-columns: 120px 250px 120px 90px 100px 130px", transactions_rules)
        self.assertIn("justify-content: center; column-gap: 40px", transactions_rules)
        self.assertIn("padding-right: 16px; padding-left: 16px", transactions_rules)
        self.assertNotIn("table-layout: fixed", transactions_rules)
        self.assertNotIn("nth-child", transactions_rules)
        self.assertNotIn("width:", "\n".join(line for line in transactions_rules.splitlines() if "th:nth-child" in line))
        self.assertNotIn("repeat(6, 1fr)", transactions_rules)
        self.assertNotIn("justify-content: space-between", transactions_rules)
        self.assertNotIn("64ch", transactions_rules)
        self.assertNotIn("min-width: 1400px", transactions_rules)
        ...
        self.assertNotIn("text-overflow: ellipsis", transactions_rules)
        mobile = styles[styles.index("@media (max-width: 760px)"):]
        self.assertIn(".transactions-page__table .data-table { min-width: 1042px; }", mobile)
