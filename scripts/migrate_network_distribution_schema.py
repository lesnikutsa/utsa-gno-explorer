...
from scripts.init_database import (FINAL_SCHEMA_EXPECTATIONS, PRE_GOVERNANCE_SCHEMA_EXPECTATIONS,
    PRE_NETWORK_DISTRIBUTION_EXPECTATIONS, PRE_TRANSACTION_PARTICIPANT_EXPECTATIONS,
...
def migrate(database_url: str, connect=None):
    ...
    with connect(database_url) as connection, connection.cursor() as cursor:
        ...
        stages = {"pre-governance": PRE_GOVERNANCE_SCHEMA_EXPECTATIONS}
        if GOVERNANCE_TABLES <= target_snapshot["tables"]:
            stages = {
        ...
        validate_one_of_exact_schema_stages(target_snapshot, stages)
        connection.commit()
