...
def run(args):
    ...
    with psycopg.connect(config.database_url) as connection:
        ...
        try:
            ...
            result=collect_distribution(config.chain_id, sources, timeout=config.geo_timeout, geo_resolver=geo_resolver)
            if args.dry_run:
            ...
            return 0
        finally: release_lock(connection, config.chain_id)

def main(argv=None):
    args=parser().parse_args(argv)
    try: return run(args)
    except (ValueError, RuntimeError, AllSourcesFailed) as exc:
