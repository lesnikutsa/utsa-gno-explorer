...
def backfill_height(database, height: int, probes) -> None:
    ...
    for probe in suitable_rpc_probes(probes):
        try:
            ...
            results = parse_execution_results(height, results_payload, len(block["transactions"]))
            with database.connect() as connection:
                with connection.cursor() as cursor:
                    cursor.execute("SELECT block_hash_hex FROM blocks WHERE height=%s", (height,))
                    row = cursor.fetchone()
                    ...
                    cursor.execute("SELECT tx_index FROM transactions WHERE block_height=%s ORDER BY tx_index", (height,))
                    if [int(item[0]) for item in cursor.fetchall()] != list(range(len(results))):
                        raise RpcError(f"Local transaction index mismatch at height {height}")
                    for result in results:
                        cursor.execute("""
                            INSERT INTO transaction_execution_results(block_height,tx_index,execution_status,gas_wanted,gas_used,error_text,log_text,info_text,data_base64,events,raw_result,source_rpc_endpoint_id)
                            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s::jsonb,%s::jsonb,%s)
                            ON CONFLICT (block_height,tx_index) DO NOTHING
                        """, (height,result["tx_index"],result["execution_status"],result["gas_wanted"],result["gas_used"],result["error_text"],result["log_text"],result["info_text"],result["data_base64"],database_json(result["events"]),database_json(result["raw_result"]),database.selected_rpc_endpoint_id))
                connection.commit()
            return
